...
\end{equation}
What matters from \eqref{eq: proof_bound} is that since $h$ is a decreasing function of $t$, there exists $T$ such that $\delta(c_{T}) = \delta_{m}$. We are guaranteed that the algorithm will not stop later than $T$ since by definition of $\delta(c_{T})$ and $\gamma$:
